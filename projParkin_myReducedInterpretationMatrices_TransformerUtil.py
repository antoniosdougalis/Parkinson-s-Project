...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        res = x
        attn_output, attention_weights = self.mha( self.AttNorm(x) )
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # embed the data f
        x = self.embedding(x)
        embedding_output = x # store the embedding matrices
        ...
        # initialise outputs
        n_layers = len(self.enc_layers)
        n_heads = self.num_heads
        batch_size = x.shape[0]
        n_channels = x.shape[1]
        d_model = x.shape[2]
        
        # should be (num_layers, batch_size, seq_len, d_model)
        all_layer_embeddings = torch.zeros(
            n_layers,
            batch_size,
            n_channels,
            d_model,
        ...
        # should be (num_layers, batch_size, num_heads, seq_len, seq_len)
        ...
        for nl, layer in enumerate(self.enc_layers):
            x, attention_weights = layer(x)
            all_layer_embeddings[nl, ...] = x
            all_attention_weights[nl, ...] = attention_weights
...
class EmT(nn.Module):
    ...
    def forward(self, x):
        ...
        x, embedding_output, all_layer_embeddings, all_attention_weights = self.encoder(x)
        
        # x = x[:, -1, :]  # Get the last output for classification
        # x = x.mean(dim=1)  # Get the mean of the channels as the output
        
        scores = self.pool(x)                      # (batch_size, channels, 1)
        weights = torch.softmax(scores, dim=1)     # channel importance (sums to 1)
        x = (x * weights).sum(dim=1)               # weighted sum instead of mean
