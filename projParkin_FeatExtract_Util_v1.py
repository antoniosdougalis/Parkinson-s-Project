...
#%% Preprocessing data: Band pass, CAR, ICA and epoching

from scipy.signal import butter, filtfilt
from sklearn.decomposition import FastICA
from scipy.stats import kurtosis

def plot_EEG(EEGdata, fs, title = None, ch_names=None, max_channels=None ):
    '''
    Plots the data of any multichannel signal
    
    INPUTS:
    EEGdata : (numpy array) channels x sample points
    fs : (float) sampling frequency in Hz
    title : (string) title for the figure.
    ch_names : (list) list of string s holding channel namesal
    max_channels : (int) number of channelks to plot (defaults to five random)
    ...
    n_channels, n_timepoints = EEGdata.shape
    time = np.arange(n_timepoints) / fs
    offset = 0.000001  # spacing between channel traces
    
    for i in range(min(n_channels, max_channels)):
        label = ch_names[i] if ch_names else f"Ch {i+1}"
        plt.plot(time, EEGdata[i] + i * offset, label=label)
...
def calculate_EEG_epochs(EEGdata, fs, epoch_duration=5, overlap=1): 
                  
    """
    Preprocesses EEG data: epoching.

    Parameters:
    - EEGdata: 2D NumPy array (channels x timepoints)
    - fs: Sampling frequency (Hz)
    - epoch_duration: Epoch length in seconds
    - overlap: Overlap in seconds between epochs

    Returns:
    - number of epochs that EEG is broken down: 3D NumPy array (n_epochs x channels x samples_per_epoch)
    """
    
    n_channels, n_timepoints  = EEGdata.shape
    if n_channels>n_timepoints:
        EEGdata = EEGdata.T
        n_channels, n_timepoints  = EEGdata.shape
        
    ### --- Epoching --- ###
    epoch_samples = epoch_duration * fs
    step_samples = (epoch_duration - overlap) * fs

    n_epochs = (n_timepoints - epoch_samples) // step_samples + 1
    
    print(f"Preprocessing complete. Returned shape: {n_epochs}")
    
    return n_epochs

def preprocess_EEG(EEGdata, fs, ch_names=None, 
                   low_freq=1, high_freq=45, 
                   epoch_duration=5, overlap=1,
                   power_percentile=90, kurt_percentile=90,
                   plot=True):
    """
    Preprocesses EEG data with band-pass filtering, CAR, ICA artifact removal, and epoching.

    Parameters:
    - EEGdata: 2D NumPy array (channels x timepoints)
    - fs: Sampling frequency (Hz)
    - ch_names: List of channel names (optional)
    - low_freq, high_freq: Band-pass filter cutoff frequencies (Hz)
    - epoch_duration: Epoch length in seconds
    - overlap: Overlap in seconds between epochs
    - power_percentile: Power threshold for ICA artifact rejection (0–100)
    - kurt_percentile: Kurtosis threshold for ICA artifact rejection (0–100)
    - plot: Whether to plot signals at each stage

    Returns:
    - epochs: 3D NumPy array (n_epochs x channels x samples_per_epoch)
    """

    if not isinstance(EEGdata, np.ndarray):
        raise ValueError("EEGdata must be a numpy array.")
    if EEGdata.ndim != 2:
        raise ValueError("EEGdata must be 2D (channels x timepoints).")
    if not isinstance(fs, int) or fs <= 0:
        raise ValueError("Sampling rate 'fs' must be a positive integer.")
    ...
    if ch_names is not None:
        if not isinstance(ch_names, list) or not all(isinstance(ch, str) for ch in ch_names):
            raise ValueError("ch_names must be a list of strings.")
        if len(ch_names) != n_channels:
            raise ValueError("Length of ch_names must match number of EEG channels.")
            
    if plot:
        plot_EEG(EEGdata, fs, "Raw EEG Signal", ch_names)

    ### --- Band-pass Filter --- ###
    def bandpass_filter(data, fs, low, high, order=2):
        nyq = 0.5 * fs
        b, a = butter(order, [low / nyq, high / nyq], btype='band')
        return filtfilt(b, a, data, axis=1)

    EEGdata = bandpass_filter(EEGdata, fs, low_freq, high_freq)

    if plot:
        plot_EEG(EEGdata, fs, f"After Band-pass Filtering ({low_freq}-{high_freq} Hz)", ch_names)
        
    
    ### --- Common Average Referencing --- ###
    EEGdata = EEGdata - np.mean(EEGdata, axis=0)

    if plot:
        plot_EEG(EEGdata, fs, "After Common Average Referencing (CAR)", ch_names)
        

    ### --- ICA with Automatic Artifact Detection --- ###
    ica = FastICA(n_components=n_channels, random_state=0, max_iter=300)
    sources = ica.fit_transform(EEGdata.T).T
    
    mixing_matrix = ica.mixing_  # shape: (n_channels, n_components)

    component_power = np.sum(mixing_matrix ** 2, axis=0)
    component_kurt = kurtosis(sources, axis=1)
    
    # # ---------- spectral muscle ratio ----------
    # def band_power(sig, sfreq, fmin, fmax):
    #     f, pxx = welch(sig, sfreq, nperseg=sfreq*2)
    #     idx = np.logical_and(f >= fmin, f <= fmax)
    #     return np.trapz(pxx[idx], f[idx])

    # muscle_ratio = np.array([
    #     band_power(sources[c], fs, 25, 45) /
    #     band_power(sources[c], fs, 1, 15)
    #     for c in range(sources.shape[0])
    # ])
    
    # # ---------- robust thresholds ----------
    # z_pow  = (comp_power - np.median(comp_power)) / median_abs_deviation(comp_power)
    # z_kurt = (comp_kurt  - np.median(comp_kurt)) / median_abs_deviation(comp_kurt)

    # artifact_idx = np.where(
    #     (z_pow  > z_thresh) |
    #     (z_kurt > z_thresh) |
    ...
    artifact_indices = np.where(
        (component_power > power_thresh) | (component_kurt > kurt_thresh)
    )[0]

    print(f"Identified {len(artifact_indices)} ICA artifact component(s): {artifact_indices.tolist()}")

    sources[artifact_indices, :] = 0
    EEGdata = ica.inverse_transform(sources.T).T

    if plot:
        plot_EEG(EEGdata, fs, "After ICA Artifact Removal", ch_names)
    ...
    for i in range(n_epochs):
        ...
        if end <= n_timepoints:
            epochs.append(EEGdata[:, start:end])

    if not epochs:
        raise ValueError("No epochs could be created with the given duration and overlap.")

    epochs = np.stack(epochs)  # shape: (n_epochs, n_channels, samples_per_epoch)
    

    if plot:
        time = np.arange(epoch_samples) / fs
        plt.figure(figsize=(12, 4))
        for i in range(min(n_channels, 5)):
            plt.plot(time, epochs[0, i] + i * 0.000001, label=ch_names[i] if ch_names else f"Ch {i}")
        plt.title("Example Epoch (First Epoch)")
        plt.xlabel("Time (s)")
        plt.tight_layout()
        plt.legend()
        plt.show()

    print(f"Preprocessing complete. Returned shape: {epochs.shape}")
    
    return epochs, sources, component_power, component_kurt

# epoched_EEG, ICA_sources, component_power, component_kurt = preprocess_EEG(EEGdata, fs, ch_names=ch_names, 
#                    low_freq=1, high_freq=45, 
#                    epoch_duration=5, overlap=1,
#                    power_percentile=85, kurt_percentile=85,
#                    plot=True)
    
#%% visualise the ICA components

import seaborn as sns

def inspect_ICA(epoched_EEG, ICA_sources, component_power, component_kurt):
    N_to_plot = min(epoched_EEG.shape[1], epoched_EEG.shape[1])
    fig, axes = plt.subplots(N_to_plot, 1, figsize=(12, 2 * N_to_plot), sharex=True)
    for i in range(N_to_plot):
        axes[i].plot(ICA_sources[i])
        axes[i].set_ylabel(f"IC {i}")
        axes[i].set_title(f"IC {i}: power={component_power[i]:.2f}, kurt={component_kurt[i]:.2f}")
    plt.xlabel("Time (samples)")
    plt.tight_layout()
    plt.suptitle("ICA Component Time Courses", y=1.02)
    plt.show()
...
def plot_ICA_Results(component_power, component_kurt, power_thres, kurt_thres):
    
    fig,_ = plt.subplots(1,2, figsize=(12, 5))
    
    plt.subplot(1, 2, 1)
    sns.histplot(component_power, bins=20, kde=True)
    plt.axvline(np.percentile(component_power, power_thres), color='r', linestyle='--', label='Power threshold')
    plt.title("ICA Component Power")
    plt.xlabel("Power")
    plt.legend()
    
    plt.subplot(1, 2, 2)
    sns.histplot(component_kurt, bins=20, kde=True)
    plt.axvline(np.percentile(component_kurt, kurt_thres), color='r', linestyle='--', label='Kurtosis threshold')
